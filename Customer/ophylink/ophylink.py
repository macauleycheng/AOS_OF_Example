...
class ophylink(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]
	
    def __init__(self, *args, **kwargs):
        super( ophylink, self).__init__(*args, **kwargs)
    ...
    def switch_features_handler(self, ev):
        ...
        actions = [parser.OFPActionSetField(vlan_vid=10), parser.OFPActionGroup(0x000a0002) ]
        buckets = [parser.OFPBucket(weight=100, watch_port=0, watch_group=0, actions=actions)]
        self.add_group(datapath, ofproto.OFPGT_INDIRECT, 0x10000001, buckets)
        
        #add acl table
        actions = [parser.OFPActionGroup(0x10000001)]
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, actions)]
        match = parser.OFPMatch()
        match.set_in_port(1)
        self.add_flow(datapath ,60 ,1, match , inst)  

        #add vlan flow, tagged vlan 1, inport 1
        ...
        #add vlan flow, untagged, inport 1     
        actions = [parser.OFPActionSetField(vlan_vid=1)]
        inst = [parser.OFPInstructionActions(ofproto.OFPIT_APPLY_ACTIONS, actions),
                parser.OFPInstructionGotoTable(20)]
        match = parser.OFPMatch()
