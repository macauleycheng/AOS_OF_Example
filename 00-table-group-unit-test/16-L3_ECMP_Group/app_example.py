...
class app_example(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        actions = [parser.OFPActionSetField(vlan_vid=2),
                   parser.OFPActionSetField(eth_src="00:00:00:22:44:66"), parser.OFPActionGroup(0x00020001)]
        buckets = [parser.OFPBucket(weight=100, watch_port=0, watch_group=0, actions=actions)]
        self.add_group(datapath, ofproto.OFPGT_INDIRECT, 0x20000001, buckets)

        #create ecmp_group, type is select
        actions = [parser.OFPActionGroup(0x20000001)]
        buckets = [parser.OFPBucket(weight=100, watch_port=0, watch_group=0, actions=actions)]
        self.add_group(datapath, ofproto.OFPGT_SELECT, 0x70000001, buckets)
