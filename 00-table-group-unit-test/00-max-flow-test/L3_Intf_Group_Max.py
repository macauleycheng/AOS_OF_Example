...
class L3IntfGroupMaxTest(app_manager.RyuApp):

    _CONTEXTS = {'dpset': dpset.DPSet}
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(L3IntfGroupMaxTest , self).__init__(*args, **kwargs)
    ...
    def create_max_l3_intf_group(self, datapath):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
        ...
        print 'Start to test l3 intf group'
        for i in range(0, entry_num, 1):
            v = (i % max_vlan_id) + 1
            group_id = 0x50000000 + i
            actions = [parser.OFPActionSetField(vlan_vid=v),parser.OFPActionSetField(eth_src=change_mac(sa_start, i)),
                       parser.OFPActionGroup((v <<16) + 1)]
            buckets = [parser.OFPBucket(weight=100, watch_port=0, watch_group=0, actions=actions)]
            self.add_group(datapath, ofproto.OFPGT_INDIRECT, group_id, buckets)
            time.sleep(1)
        print 'Finish to full l3 intf group testing'
    ...
    def group_stats_reply_handler(self, ev):
        global l3_intf_group_curr_num
        print 'group stats reply handler'
        print('l3_intf_group_curr_num is %d' % l3_intf_group_curr_num)
        for stat in ev.msg.body:
            ...
            #               stat.duration_nsec))
            if ((stat.group_id & 0x50000000) == 0x50000000):
                l3_intf_group_curr_num += 1;

        if (l3_intf_group_curr_num != entry_num):
            print ('Get group stats reply, not reach max L3 interface group yet, current entry is %d, max is %d' % (l3_intf_group_curr_num, entry_num))
        else:
            print ('Get group stats reply, success to test max L3 interface group, current entry is %d, max is %d' % (l3_intf_group_curr_num, entry_num))
    ...
    def handler_datapath(self, ev):
        ...
        if ev.enter:
            ...
            self.create_max_l3_intf_group(ev.dp)
