...
class L3EcmpGroupMaxTest(app_manager.RyuApp):

    _CONTEXTS = {'dpset': dpset.DPSet}
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(L3EcmpGroupMaxTest , self).__init__(*args, **kwargs)
    ...
    #
    def create_max_l3_ecmp_group(self, datapath):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
        # create l2 intf group
        for v in range(1, bucket_num+1, 1):
        ...
        l3_ecmp_buckets = []
        for i in range(0, bucket_num, 1):
            v = (i % bucket_num) + 1
            group_id = 0x20000000 + (v<<16) + ((i/bucket_num)+1)
            ...
            buckets = [parser.OFPBucket(weight=100, watch_port=0, watch_group=0, actions=actions)]
            self.add_group(datapath, ofproto.OFPGT_INDIRECT, group_id, buckets)
            tmp_l3_ecmp_actions = [parser.OFPActionGroup(group_id)]
            l3_ecmp_buckets.append(parser.OFPBucket(weight=100,
                                                    watch_port=ofproto.OFPP_ANY,
                                                    watch_group=ofproto.OFPG_ANY,
                                                    actions=tmp_l3_ecmp_actions))
            time.sleep(1)

        #print l3_ecmp_buckets
        # create l3 ecmp group
        # ./dpctl tcp:192.168.1.10:6633 group-mod cmd=add,type=sel,
        # group=0x70000001 group=any,port=any,weight=0 group=0x20000001
        print 'Start to test full l3 ecmp group'
        for i in range(0, entry_num, 1):
            group_id = 0x70000000 + i + 1
            #print 'group_id=%x' % group_id
            self.add_group(datapath, ofproto.OFPGT_SELECT, group_id, l3_ecmp_buckets)
            time.sleep(1)
        print 'Finish to full l3 ecmp group'
    ...
    def group_stats_reply_handler(self, ev):
        global l3_ecmp_group_curr_num
        print 'group stats reply handler'
        print('l3_ecmp_group_curr_num is %d' % l3_ecmp_group_curr_num)
        for stat in ev.msg.body:
            ...
            #               stat.duration_nsec))
            if ((stat.group_id & 0x70000000) == 0x70000000):
                l3_ecmp_group_curr_num += 1;

        if (l3_ecmp_group_curr_num != entry_num):
            print ('Get group stats reply, not reach max L3 ECMP group yet, current entry is %d, max is %d' % (l3_ecmp_group_curr_num, entry_num))
        else:
            print ('Get group stats reply, success to test max L3 ECMP group, current entry is %d, max is %d' % (l3_ecmp_group_curr_num, entry_num))
    ...
    def handler_datapath(self, ev):
        ...
        if ev.enter:
            ...
            self.create_max_l3_ecmp_group(ev.dp)
