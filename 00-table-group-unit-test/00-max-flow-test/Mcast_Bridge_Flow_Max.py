...
class McastBridgeFlowMaxTest(app_manager.RyuApp):

    _CONTEXTS = {'dpset': dpset.DPSet}
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(McastBridgeFlowMaxTest , self).__init__(*args, **kwargs)
    ...
    # eth_dst=01:00:5e:01:01:01,vlan_vid=2 write:group=0x30020001 goto:60
    def create_max_l2_mcast_bridge_flow(self, datapath):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
        ...
        for v in range(1, max_vlan_id+1, 1):
            ...
            #print buckets
            self.add_group(datapath, ofproto.OFPGT_ALL, (0x30000001 + (v<<16)), buckets)
            #print 'Finish to create L2 multicast group %x' % (0x30000000 + (v<<16) + i)
            time.sleep(sleep_time)
        print "finish create L2 multicast group"

        # create full mcast bridge flow
        print 'Start to test full mcast bridge flow'
        for i in range(0, entry_num, 1):
            v = (i%max_vlan_id) + 1
            actions = [parser.OFPActionGroup(0x30000001 + (v<<16))]
            ...
            time.sleep(sleep_time)
        print 'Finish to full mcast bridge flow testing'
    ...
    def handler_datapath(self, ev):
        ...
        if ev.enter:
            ...
            self.create_max_l2_mcast_bridge_flow(ev.dp)
